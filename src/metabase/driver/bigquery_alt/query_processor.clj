(ns metabase.driver.bigquery_alt.query-processor
  (:require [clj-time
            ...
            [metabase
             [driver :as driver]
             [util :as u]]
            [metabase.driver.bigquery_alt.common :as bigquery.common]
...
(defmethod sql.qp/unix-timestamp->timestamp [:bigquery_alt :seconds] [_ _ expr]
  (hsql/call :timestamp_seconds expr))

(defmethod sql.qp/unix-timestamp->timestamp [:bigquery_alt :milliseconds] [_ _ expr]
...
(defmethod sql.qp/->honeysql [:bigquery_alt Date]
  [_ date]
  (hsql/call :timestamp (hx/literal (du/date->iso-8601 date))))

(defmethod sql.qp/->honeysql [:bigquery_alt :time]
  [driver [_ value unit]]
  (->> value
...
(defmethod driver/date-add :bigquery_alt
  [_ dt amount unit]
...
(defmethod sql.qp/current-datetime-fn :bigquery_alt
  [_] :%current_timestamp)
